...
options:
  collect:
    description: Dictionary of mount types and subtypes to return.
    type: list
    elements: str
    required: true
    choices:
      ...
      - zfs
  timeout:
    description: The maximum number of seconds to qeury for each mount type in O(collect).
    default: 20
    type: int
  timeout_handler:
    description:
...
EXAMPLES = """
- name: collect nfs and fuse filesystem subtype mounts
  remote_mount_facts:
    collect:
      - nfs
      - fuse
...
# Regular expressions to match a line from mount
...
# When possible, don't parse mount output
...
def list_current_mounts(module: AnsibleModule) -> list[dict[str, str]]:
    ...
    Checks the following sources for current mounts:
      ...
      * /proc/mounts
      * /etc/mnttab (Solaris)
    If no information if sound in the files, attempts to parse the output from mount.
    """
